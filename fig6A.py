...
# ECM features
ecm_features = np.array([
    extract_lower(m)
    for m in fc_matrices
])

# LEC features
lec_features = np.array([
    extract_lower(
        np.log(np.abs(m) + 1e-12)
    )
    for m in fc_matrices
])
...
bg_ecm = biswas_ghosh_test(
    ecm_features[labels == 1],
    ecm_features[labels == 2]
)
...
bg_lec = biswas_ghosh_test(
    lec_features[labels == 1],
    lec_features[labels == 2]
)
...
with open("hypothesis_test_results_LW.txt", "w") as f:
    ...
    f.write("Hypothesis Test Results for EEG Data – Ledoit-Wolf (LW)\n\n")
...
# PGA visualization using PCA implementation
pga = PCA(n_components=2)
...
print("Figure saved as fig6_A.png")