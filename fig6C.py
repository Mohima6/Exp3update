...
def load_balanced_data(data_folder, samples_per_class=45):
    ...
    for epochs, labels in zip(all_epochs, all_labels):
        X_truncated.append(epochs[:, :, :min_time])
...
def compute_fc_matrix_scm(trial):
    """
    Computes an empirical correlation matrix from a single trial.
...
def run_pipeline_scm(data_folder, samples_per_class=45):
    ...
    for trial in X_raw:
        fc = compute_fc_matrix_scm(trial)          # empirical correlation matrix
    ...
    plt.suptitle("(C) Empirical Correlation (SCM)", fontsize=14)
    plt.tight_layout()
    plt.savefig("figure6_C.png", dpi=150)
    plt.show()
    print("saved as figure6_C.png")
    print("\n--- Hypothesis Test Results (ECM geometry, SCM) ---")
    ...
    print("\n--Hypothesis Test Results (LEC geometry, SCM) ---")
    ...
    with open("hypothesis_test_results_SCM.txt", "w") as f:
        f.write("Hypothesis Test Results for EEG Data using SCM (empirical correlation) – 45 samples per class\n")
    ...
    print("\nResults saved to hypothesis_test_results_SCM.txt")
if __name__ == "__main__":
    run_pipeline_scm("processed_data_epochs", samples_per_class=45)