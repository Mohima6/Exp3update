...
def load_balanced_data(data_folder, samples_per_class=45):
    ...
    for file in sorted(os.listdir(data_folder)):
        if file.endswith("_epochs.npy"):
            ...
            epochs = np.load(
                os.path.join(data_folder, f"{base}_epochs.npy")
            )
            labels = np.load(
                os.path.join(data_folder, f"{base}_labels.npy")
            )
...
def compute_fc_matrix_scm(trial):
    data = trial.T
...
def wasserstein_test(X, Y, n_perm=1000):
    ...
    return p_value
def run_pipeline_scm(data_folder, samples_per_class=45):
    X_raw, y = load_balanced_data(
        data_folder,
        samples_per_class
    )
    print("Loaded data:", X_raw.shape)
    ecm_features = []
    lec_features = []
    for trial in X_raw:
        corr = compute_fc_matrix_scm(trial)
    ...
    plt.suptitle(
        "(C) Empirical Correlation Matrix (SCM)",
        fontsize=14,
        fontweight='bold'
    )
    plt.tight_layout()
    plt.savefig(
        "figure6_C_corrected.png",
        dpi=150
    )
    plt.show()
    print("\nSaved as figure6_C_corrected.png")
    print("\nECM Geometry Tests")
    ...
    print("Biswas-Ghosh p-value:", round(bg_ecm, 4))
    print("MMD p-value:", round(mmd_ecm, 4))
    print("Wasserstein p-value:", round(wass_ecm, 4))
    print("\nLEC Geometry Tests")
    ...
    print("Biswas-Ghosh p-value:", round(bg_lec, 4))
    print("MMD p-value:", round(mmd_lec, 4))
    print("Wasserstein p-value:", round(wass_lec, 4))
    with open(
        "hypothesis_test_results_SCM_corrected.txt",
        "w"
    ) as f:

        f.write(
            "Hypothesis Test Results using SCM\n\n"
        )
        f.write("ECM Geometry\n")
        f.write(f"Biswas-Ghosh: {bg_ecm:.4f}\n")
        f.write(f"MMD: {mmd_ecm:.4f}\n")
        f.write(f"Wasserstein: {wass_ecm:.4f}\n\n")
        f.write("LEC Geometry\n")
        f.write(f"Biswas-Ghosh: {bg_lec:.4f}\n")
        f.write(f"MMD: {mmd_lec:.4f}\n")
        f.write(f"Wasserstein: {wass_lec:.4f}\n")
    print(
        "\nSaved hypothesis_test_results_SCM_corrected.txt"
    )
